...
- **Save / load graph state.** `Save` downloads a small `<graph>.state.json` capturing the logical view — seeds (pattern box + pinned), tags pinned to the bar, hidden/revealed nodes, the `↑inputs`/`↓finals`/Filter/Link toggles, menu fold state, and theme. `Load` opens a file picker and re-applies it (re-running the filter, which re-frames). The file is portable: it survives `make graph` regenerating the HTML, so you can commit it, share it, or restore a saved view later. Loading a state saved from a differently-titled graph asks for confirmation first.
