...
- **`? Help` overlay.** A `? Help` button on the menu bar opens a controls/keyboard cheat-sheet — the two highlight sets (seeds / search results / both), the Search box, picking seeds, Tracing, the on-node action bar, menus, and the `/` · `F` · `Esc` keys. Closes on `✕`, backdrop click, or `Esc`.
- **Top bar reorganized into a menu bar.** The flat row of buttons became **File ▾** (Export PNG/SVG, Save / Save As / Open state), **View ▾** (Fit — keyboard `F` — + Light/Dark background), **Tracing ▾** (`← inputs` / `→ finals` / `↔ Link`), and **Actions ▾** (Reset / Clear / Unhide / Fold-Unfold all), plus a **Filter** slide-switch. The project title + stats sit on the top row with the **Search** box at right; a **Seed:** cluster (**Targets ▾**, type legend, **Tags ▾**, **Seeds ▾**) sits below. (`Menu` → `Targets ▾`; the dropdowns are anchored under their buttons, one open at a time, close on outside-click / `Esc`.)
- **Seeds vs. Search results — two distinct sets.** *Seeds* (picked anchors — double-click / pickers) render **solid yellow**; *search results* (Search-box matches) render **dashed orange**; a node that's **both** is **dashed yellow**. **Tracing applies to seeds only** — connect-between and the `↑inputs`/`↓finals`/`↔Link` modifiers extend from seeds; search results are shown and highlighted but never extend the trace. **Seed results** / **Unseed results** buttons add/remove all current matches to/from the seeds, and an **in view** switch scopes the search to nodes already on screen. A seed key on the top row labels the three states; the hover tooltip names a node's kind.
- **Search box matches name, `HELP`, and `DESC`** (not just target names) — bare tokens are substrings that reach the help/desc prose; globs (`*_alpha_*`) still target the name.
- **`Seeds ▾` menu** — a searchable picker (same panel as Targets/Tags) listing the current seeds as toggles; toggling applies to the graph immediately, and a row stays listed until the panel is reopened so a mis-click can be undone.
- **`Add visible` / `Remove visible`** in every node picker (Targets / Tags-drilled / types / Seeds) bulk-seed or bulk-unseed exactly the rows currently shown — **rows under folded groups are left untouched**.
- **Save / Save As / load graph state.** The state is the logical view — seeds (pattern box + pinned), tags pinned to the bar, hidden/revealed nodes, the `↑inputs`/`↓finals`/Filter/Link toggles, menu fold state, and theme — written to a small `<graph>.state.json`. **`Save As`** chooses a new name/location; **`Save`** overwrites the last-saved file. Where the File System Access API is available (Chromium), `Save As` opens a real OS dialog and `Save` overwrites the chosen file in place; elsewhere it falls back to a filename prompt + download. **`Load`** opens a file picker and re-applies the state (re-running the filter, which re-frames). The file is portable: it survives `make graph` regenerating the HTML, so you can commit it, share it, or restore a saved view later. Loading a state saved from a differently-titled graph asks for confirmation first.
...
- **Add siblings.** A node that's one target of a multi-target rule now shows an extra **`⧉`** button on its hover action bar; clicking it seeds *all* of that rule's targets (its co-outputs) at once — handy when you're working on one output and want the rest of the set in the trace. The button is hidden for single-target nodes and inputs. Backed by a new per-node `rule` index in `.makexx_graph.json` (shared by a rule's targets, `-1` for inputs/tools).
...
- **Fix: hiding / revealing a node no longer resets the camera.** The dagre relayout that runs after every edit had auto-`fit` on, so a single hide (or step-by-step ⊕ reveal) re-framed the whole view — typically zooming all the way out. Incremental edits now lay out with `fit:false`, keeping your zoom/pan; only seed/filter changes (a genuinely new subgraph) re-frame, and `Fit` still re-frames on demand.
- **`Reset` button (keep seeds).** Sits next to `Clear`. Drops the accumulated step-by-step reveals, manual hides, and modifier toggles (`← inputs` / `→ finals` / `↔ Link`, Filter back on) but **keeps the seeds** (pattern box + pinned chips) — so you can get back to the clean seed-filtered subgraph without re-picking everything. `Clear` still wipes seeds too.
- **Type & tag pickers.** Click a legend type (input / intermediate / final / phony / tool) to open a popup listing every node of that type, with a filter box; click rows to add them as seeds (toggle; `●` marks added) or **"Add all shown"**. Lets you pull, say, all final outputs (or just the ones matching a filter) into the trace without hunting for their names. `Esc`/`✕` closes.
- **Tag browser.** The toolbar carries a single **`Tags ▾ (N)`** button instead of a chip per tag (which would overflow). It opens a searchable list of every `#tag` with node counts; from a row you can **drill into** that tag's node list (same picker as above, with a **`← Back`** button to return to the list) *or* **`pin to bar`** to promote just that tag to a quick-access toolbar chip (each pinned chip has its own `×`). So the bar only ever holds the handful of tags you chose.
- **SVG export.** An `SVG` toolbar button (next to `PNG`) downloads the current visible/traced subgraph as a vector `.svg` on the active background — scales to any size without pixelation, good for print or further editing. Vendors the `cytoscape-svg` extension (embedded offline like the rest); PNG and SVG now share one `download()` helper.
- **Target menu.** A `Menu` toolbar button opens the same grouped target list you see in `makexx -i` — documented targets under their `MENU` groups (nested via `/`, with auto-created parent section headers), foldable, with a folded **Undocumented** bucket for targets that have no `HELP`. The filter box matches name, description, or group; clicking a target toggles it as a seed (`●` marks selected). Lets you trace a target by browsing the menu instead of recalling its exact name. **Honors the initial fold state of `makexx -i`**: groups declared `MENU(..., FOLDED)` start folded (their fold state is now emitted into `.makexx_graph.json` as `folded_groups`), and — as in the TUI — search ignores folds so matches always surface.
- **Light / dark toggle.** A `☀ Light` / `🌙 Dark` button flips the whole viewer. The page chrome swaps via a `html.light` CSS-var override; the canvas — which can't use CSS `var()` — carries two literal palettes (`THEMES.dark` / `THEMES.light`) re-applied with `cy.style().fromJson(...)`, so toggling re-skins node fills / text / edges **without moving anything** (positions, filter and layout are preserved). The type-accent borders and seed yellow read on both backgrounds. PNG export already follows the active background, so light-mode shots come out on white.
