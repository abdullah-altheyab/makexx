...
- **`TOOL` install hints via new `TOOLDESC`.** `TOOLDESC("name", "install hint")` declares a tool **and** records its install hint (URL, package-manager command, or free text) in one call — no separate `TOOL` needed. Works on a rule (`rule << TOOLDESC(...)`) or the makefile (`mf << TOOLDESC(...)`); mf-level hints win over rule-level on conflict. A braced `TOOLDESC({"a", "b"}, "hint")` applies the same hint to several tools — for multiple binaries that ship in one package. `TOOL` keeps its variadic and braced multi-tool forms — `TOOL("a", "b", "c")` and `TOOL({"a", "b"})` — and `mf << TOOL(...)` now declares project-level tools (attached to no rule) that still show up in `check_tools` and AGENTS.md. (The hint is deliberately *not* a second string argument to `TOOL`, which would be ambiguous with declaring two tools.)
...
- **AGENTS.md gains a `## Tools` section** listing every declared tool and pointing agents at `make check_tools`. Tools that **share an install hint are grouped onto one line** (with a note that they come from one package / one install step), so N binaries from a single package read as one install action instead of N separate ones; tools with distinct hints stay on their own lines. `check_tools` is also listed under **Built-in targets**, and the section intro states the command reports *which* tools are missing and prints each install hint. The "Usage & timing data" section now also points at `makexx --stats` for a ready-made per-rule summary (incl. never-run targets) instead of hand-parsing `.makexx_hits`. The generated context file opens with a do-not-edit warning, uses consistent `<<` chaining style, and its DSL quick reference now contrasts project-level `mf << TOOLDESC(...)` (documents the tool, **not** a per-target prereq) with rule-level `<< TOOL` / `<< TOOLDESC` (creates the mtime-tracked edge). `mf.profile` / `mf.silent` / `mf.context_filename` / `mf.preamble` are surfaced in the AGENTS.md quick-reference and the starter template.

### Interactive dependency graph

- **Fix: rule nodes now inherit their rule's MENU group (and HELP).** A synthetic rule node was emitted to `.makexx_graph.json` without a `group`, so it floated outside its group box in the viewer instead of sitting with the targets it belongs to. It now carries the group and HELP of its rule (all of a rule's targets share them).
- **Ranker option** (View ▾). Switch dagre's rank-assignment strategy — `network-simplex` (default, most compact / minimizes edge length), `tight-tree`, or `longest-path` — to try different layerings on a busy graph. (Crossing minimization is intrinsic to dagre and always on; the ranker tunes the layering, not the crossing effort.) Saved with the view state.
- **Align inputs in a column** (View ▾, on by default). dagre pulls a source node toward whatever consumes it, so input nodes scatter across ranks (an input feeding a late stage lands deep). This pins every visible input to a single vertical line (the leftmost column) after layout, de-overlapping them along the cross-axis, so "where data enters the build" reads at a glance. Toggle off to restore dagre's natural placement; the choice is saved with the view state.
...
- **`build.sh` (the no-cmake build path) un-stale'd.** It had drifted since the graph feature: it embedded only the DSL header + starter, so `src/makexx.cpp` (which `#include`s six vendored graph-asset embeds) wouldn't compile, and it hardcoded `MAKEXX_VERSION="0.1.0"` so a `build.sh`-built binary mis-reported its version. It now embeds the full asset set (matching the `VAR` names in `CMakeLists.txt`) and derives the version from the CMake `project()` line — one source of truth. A new CI job builds via `build.sh` on Linux + macOS and asserts the binary's version matches `CMakeLists.txt` and that `--build-graph` assembles the standalone HTML, so this drift is caught automatically from now on.
...
- **Menu-bar toolbar.** The controls are organized into dropdown menus anchored under their buttons (one open at a time; close on outside-click / `Esc`): **File ▾** (Export as PNG / SVG, Save / Save As / Open state), **View ▾** (Fit [`F`], Light / Dark background), **Tracing ▾** (`← inputs` / `→ finals` / `↔ Link`), **Actions ▾** (Reset / Clear / Unhide / Fold-Unfold all) — plus a **Filter** slide-switch and a **`? Help`** overlay (a controls + keyboard cheat-sheet). The project title + live stats sit on the top row with the **Search** box (and an **in view** switch) at the right; a **Seed:** cluster — **Targets ▾**, the type **legend**, **Tags ▾**, **Seeds ▾** — sits on the row below, with a **seed key**.
- **Seeds vs. Search results — two distinct sets.** *Seeds* (picked anchors) render **solid yellow**; *Search results* (Search-box matches) render **dashed orange**; a node that's **both** is **dashed yellow** (the seed key and hover tooltip label all three). **Tracing applies to seeds only** — connect-between and the `↑inputs`/`↓finals`/`↔Link` modifiers extend from seeds; search results are shown and highlighted but never extend the trace. **Seed results** / **Unseed results** promote / remove all current matches to / from the seeds.
- **Search matches name, `HELP`, or `DESC`** (substring, or glob with `*` like `*_alpha_*`; applies on Enter). The **in view** switch scopes matching to nodes already on screen.
- **Pickers.** **Targets ▾**, **Tags ▾**, **Seeds ▾**, and each legend **type** open a shared searchable panel. **Targets ▾** mirrors `makexx -i` (documented targets under `MENU` groups nested via `/`, foldable, with a folded **Undocumented** bucket; honors `MENU(..., FOLDED)` via a new `folded_groups` array in `.makexx_graph.json`, and search ignores folds). **Tags ▾** lists `#tags` (drill into a tag's nodes, or pin a tag to the bar). **Seeds ▾** lists current seeds as toggles (applies immediately; a row stays listed until the panel reopens). Every panel has **Add visible / Remove visible** that act only on the rows currently shown — **rows under folded groups are left untouched**.
- **On-node action bar (hover)** — step-by-step **`⊕`** reveal (← parents / → children) and **`⇤`/`⇥`** reveal-all-the-way; **`⊘←`/`⊘→`/`✕`** hide upstream / downstream / this node (Actions ▸ Unhide restores); and **`⧉` add siblings** — seed *all* targets of a multi-target rule (its co-outputs), backed by a new per-node `rule` index in `.makexx_graph.json` (`-1` for inputs/tools). Seeds and search results are never hidden.
- **Export & state.** **PNG** (2×) and **SVG** (vector, via the vendored offline `cytoscape-svg`) export the current view on the active background. **Save / Save As / Open state** persist the logical view (seeds, pinned tags, hidden/revealed nodes, the toggles incl. `in view`, menu fold state, theme — not zoom/pan) to a portable `<graph>.state.json`; on Chromium via the File System Access API (a real Save-As dialog + in-place overwrite), else a filename prompt + download. The file survives `make graph` regenerating the HTML, and loading a state from a differently-titled graph asks first.
- **Light / dark background** (View ▾) — the page chrome swaps via a `html.light` CSS-var override; the canvas (which can't use CSS `var()`) carries two literal palettes (`THEMES.dark`/`light`) re-applied with `cy.style().fromJson(...)`, re-skinning fills / text / edges **without moving anything**.
- **Camera stays put on incremental edits** — hide / reveal / double-click-seed / Seed-results re-lay-out with `fit:false`, preserving your zoom/pan; only deliberate new queries (pattern box, pickers) re-frame, and `F` / Fit re-frame on demand.
- **Fix:** the "xx of yyyy targets shown" counter no longer miscounts edges as targets (it now counts visible nodes only).
