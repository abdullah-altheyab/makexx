...
- **Seeds are now clearly highlighted** — a thick **yellow border + glow halo**, drawn on top, so they stand out even when zoomed out and labels are unreadable. Applies to both pattern/tag matches and double-clicked seeds; the seed chips share the colour. **Fix:** cytoscape's stylesheet doesn't resolve CSS `var(...)`, so the seed (and every node *type*) border colour was silently dropped — all node style colours are now literal hex, so type borders render too (matching the legend).
