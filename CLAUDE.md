...
`mf.generate_with_graph()` emits `.makexx_graph.json` (the DAG + per-node `type`/`group`/`HELP`/`DESC`/`tags`/`cmds`, plus a top-level `folded_groups` array of groups declared `MENU(..., FOLDED)`); `make graph` assembles a standalone, offline `makefile_graph.html` (Cytoscape.js + dagre, vendored and embedded) and opens it. The static Graphviz `make makefile_graph.pdf` remains available.
...
- **`Menu` button** opens the same grouped target list as `makexx -i`: documented targets under their `MENU` groups (nested via `/` with auto-created parent headers), foldable, plus a folded **Undocumented** bucket for targets with no `HELP`. Its filter box matches name / description / group, and clicking a target toggles it as a seed — so you can trace by browsing the menu instead of recalling exact names. (Reuses the same picker panel as the type/tag pickers below.) It honors the TUI's initial fold state — groups declared `MENU(..., FOLDED)` start folded (emitted into `.makexx_graph.json` as a `folded_groups` array), and search ignores folds so matches still surface.
...
- **Modifiers** — `↑ inputs` (extend upstream to source files) and `↓ finals` (extend downstream to final targets), both **off by default**. A single seed + `↑ inputs` = provenance; a single seed + `↓ finals` = impact/blast-radius.
- **Step-by-step expand** — **hover a node** and two **`⊕` handles** appear on its sides (left = parents, right = children); click one to reveal just that one level of immediate predecessors / successors, accumulating, so you can walk the graph hop by hop. (Single-click pins the handles to a node; double-click seeds it — separate gestures.) Distinct from `↑inputs`/`↓finals`, which jump all the way to inputs/finals at once.
- **Seeds are outlined**; pulled-in intermediates are not. Hover shows type, `HELP`, `DESC`, `#tags`, **`makefile.cpp:<line>`** (where the rule is defined — for loop-generated rules this is the template line), and the rule's **commands** (the `cmds` array; each numbered, line-continuations collapsed, long ones truncated, capped with a `… +N more`) so a traced path explains itself. The source line is captured via `__builtin_LINE()` as a default argument on `mf.add(...)` (gcc/clang, C++17) and emitted as `srcline` per node.
- **`↔ Link` toggle** answers "how are these two related?" — with 2+ seeds it shows the **shortest path between them ignoring edge direction** (cytoscape `aStar({directed:false})`), so it finds connections through a shared ancestor / convergence point, not just a directed lineage. (The default directed "between" only keeps nodes on a directed seed→seed path, which shows nothing when neither seed derives from the other.) If a seed can't reach the first seed at all, the count line says "not linked."
- **`Filter` toggle** applies/suspends the filter *without* losing the seeds: off = the full graph with your seeds still highlighted (see them in context); on = the isolated connected subgraph. **`Clear`** wipes seeds and resets the toggles; **`Reset`** (next to it) does the same *but keeps the seeds* — it only drops the step-by-step reveals, manual hides, and modifier toggles, returning you to the clean seed-filtered subgraph. Groups still fold via Fold/Unfold all.
- **`Save` / `Save As` / `Load`** persist the logical view to a portable `<graph>.state.json` (seeds, pinned bar tags, hidden/revealed nodes, the modifier toggles, menu fold state, and theme — *not* exact zoom/pan, since load re-runs the filter and re-frames). `Save As` chooses a new name/location; `Save` overwrites the last-saved file. Where the File System Access API is available (Chromium) `Save As` opens a real OS dialog and `Save` overwrites in place; otherwise it falls back to a filename prompt + download. The file survives `make graph` regenerating the HTML, so a saved view can be committed, shared, or re-loaded later; loading a state whose recorded title differs from the current graph asks for confirmation first.
