...
- **`Save` / `Load`** persist the logical view to a portable `<graph>.state.json` (seeds, pinned bar tags, hidden/revealed nodes, the modifier toggles, menu fold state, and theme — *not* exact zoom/pan, since load re-runs the filter and re-frames). The file survives `make graph` regenerating the HTML, so a saved view can be committed, shared, or re-loaded later; loading a state whose recorded title differs from the current graph asks for confirmation first.
